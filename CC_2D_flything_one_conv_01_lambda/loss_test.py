...
class MonodepthLoss(nn.Module):
    def __init__(self,n=2,SSIM_w=0.85,disp_gradient_w=0.1,lr_w=1.0):
    ...
    def forward(self,disp_left_est,disp_right_est,target):
        ...
        # self.disp_left_est = disps_left
        # self.disp_right_est = disps_right
        # Generate images
        ...
        disp_left_loss = [torch.mean(torch.abs(
                          disp_left_smoothness[i])) / 8 ** i
                          for i in range(self.n)]
        disp_right_loss = [torch.mean(torch.abs(
                           disp_right_smoothness[i])) / 8 ** i
                           for i in range(self.n)]
